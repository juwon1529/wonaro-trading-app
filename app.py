...
st.markdown("""
    <style>
        ...
        p, div, span, li { color: #D1D4DC !important; }
        h1, h2, h3, h4, h5, h6, b, strong { color: #FFFFFF !important; font-weight: 600 !important; }
        
        /* AI 리포트 영역 디자인 (글씨 크기, 줄간격 확대) */
        .ai-report-box {
        ...
        .ai-report-box h3 { color: #26A69A !important; margin-top: 30px; margin-bottom: 15px; border-bottom: 1px solid #2A2E39; padding-bottom: 10px; }
        .ai-report-box h4 { color: #FFD600 !important; margin-top: 20px; margin-bottom: 10px; }
        .ai-report-box p, .ai-report-box li { color: #F5F5F5 !important; line-height: 1.8; font-size: 16px; margin-bottom: 10px; }
    </style>
""", unsafe_allow_html=True)

@st.cache_data(ttl=300)
def load_cached_stock_data(ticker, period="3mo"): return get_stock_data(ticker, period)

@st.cache_data(ttl=3600)
def load_pattern_data(ticker): return get_stock_data(ticker, period="2y")

@st.cache_data(ttl=600)
def load_cached_macro_data(): return get_macro_data()
...
tab1, tab2 = st.tabs(["🔍 터미널: 심층 종목 분석", "📊 터미널: 관심 종목 스캐너"])

with tab1:
    with st.form(key="search_form"):
        col_search, col_btn = st.columns([5, 1])
        with col_search: ticker_input = st.text_input("Symbol", placeholder="예: AAPL, NVDA, BTC-USD (입력 후 엔터)", label_visibility="collapsed")
        with col_btn: submit_button = st.form_submit_button("Chart & Analyze")

    if submit_button and ticker_input.strip():
        st.session_state.analyzed_ticker = ticker_input.strip().upper()
    ...
    if st.session_state.analyzed_ticker:
        ...
        if error_message or hist is None:
            ...
            with chart_col:
                ...
                fig.add_trace(go.Candlestick(x=hist_ind.index, open=hist_ind['Open'], high=hist_ind['High'], low=hist_ind['Low'], close=hist_ind['Close'], name='Price', increasing_line_color='#26A69A', decreasing_line_color='#EF5350'), row=1, col=1)
                fig.add_trace(go.Scatter(x=hist_ind.index, y=hist_ind['MA20'], line=dict(color='#FFD600', width=1.5), name='MA20'), row=1, col=1)
                if 'VWAP' in hist_ind.columns: fig.add_trace(go.Scatter(x=hist_ind.index, y=hist_ind['VWAP'], line=dict(color='#E1BEE7', width=1.5, dash='dot'), name='VWAP'), row=1, col=1)
                ...
                fig.update_layout(xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=10, b=10), height=600, template="plotly_dark", plot_bgcolor="#131722", paper_bgcolor="#131722", showlegend=False)
            ...
            with info_col:
                ...
                if rsi_val >= 70: rsi_text, rsi_color = "🔴 과매수", "#EF5350"
                elif rsi_val <= 30: rsi_text, rsi_color = "🟢 과매도", "#26A69A"
                else: rsi_text, rsi_color = "⚪ 중립", "#FFB74D"
                ...
                <div style='background-color:#1E222D; padding:15px; border-radius:8px; border: 1px solid #2A2E39; margin-bottom: 15px;'>
                    <div style='margin-bottom: 15px;'><span style='color:#8B93A6; font-size:13px; font-weight:600;'>RSI (14) STATUS</span><br><span style='font-size:20px; font-weight:bold; color:{rsi_color};'>{rsi_val} - {rsi_text}</span></div>
                    <div><span style='color:#8B93A6; font-size:13px; font-weight:600;'>NEWS SENTIMENT</span><br>
                    <div style='display:flex; justify-content:space-between;'><span style='font-size:20px; font-weight:bold; color:{sent_color};'>{sentiment_score} <span style='font-size:14px'>({sentiment_status})</span></span></div>
                    <div style='width:100%; background-color:#131722; height:8px; border-radius:4px; margin-top:8px;'><div style='width:{sent_percent}%; background-color:{sent_color}; height:100%; border-radius:4px;'></div></div></div>
                </div>
                """, unsafe_allow_html=True)
            ...
            # ==========================================
            st.markdown("---")
            st.markdown("### 🧠 AI PRO 퀀트 리포트 발간")
            
            # 버튼이 이제 전체 화면 너비에서 큼직하게 중앙을 차지합니다.
            if st.button("🚀 전체 화면으로 기간별 움직임 예측 보기", type="primary"):
                if not api_key:
                    st.error("👈 좌측 사이드바에 API 키를 입력하세요.")
                else:
                    with st.spinner(f"{ai_provider} 엔진이 심층 데이터를 분석 중입니다..."):
...
with tab2:
    ...
    with col_t2_1: st.markdown("### 📊 관심 종목 일괄 스캐너")
    with col_t2_2: 
        st.markdown("<br>", unsafe_allow_html=True)
        run_batch = st.button("목록 일괄 스캔 실행", type="primary")
        
    if run_batch:
        with st.spinner("시장 데이터를 스크래핑 중입니다..."):
            df_results = analyze_watchlist(period=period)
        if not df_results.empty:
            st.success("✅ 관심 종목 스캔 완료!")
